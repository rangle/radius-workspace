...
function AddAutoBoxFeatures(node: FrameNode,props: Properties) {
  ...
  props.align = ` ${ 
    constraintsVertical[node.counterAxisAlignItems] } ${
    constraintsHorizontal[node.primaryAxisAlignItems] }`;
...
function AddAbsolutePositionFeatures(node: RectangleNode|TextNode,props: Properties) {
  ...
  props.constraints = `${
    constraintsVertical[node.constraints.vertical] } ${
    constraintsHorizontal[node.constraints.horizontal] }`;
}
